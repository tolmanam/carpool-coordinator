...
Column
	...
	TEXT
	PRIMARY KEY
	Matrix ID of the family admin user.
	family_name
	TEXT
	NOT NULL
	Family display name.
...
Column
	...
	TEXT
	PRIMARY KEY
	Unique member ID within family.
	matrix_id
	TEXT
	REFERENCES cached_families(matrix_id)
	Owner family.
	name
	TEXT
	NOT NULL
...
Stores events fetched and parsed locally by the client before route mapping is completed. | Column | Type | Constraints | Description | |---|---|---|---| | id | TEXT | PRIMARY KEY | Event UID from .ics. | | schedule_id | TEXT | NOT NULL | References schedule configuration. | | title | TEXT | NOT NULL | Event title. | | start_time | INTEGER | NOT NULL | Unix timestamp of start time. | | end_time | INTEGER | NOT NULL | Unix timestamp of end time. |
